...
df = pd.DataFrame({'pillars' :["Teaching","Teaching","Teaching","Teaching","Teaching",
                               "Research","Research","Research",
                               "Citation", 
                               "International\nOutlook","International\nOutlook","International\nOutlook",
                               "Industry"],
                   'subweight':['reputation','student-staff ratio','doctorate-bachelor','doctorate-staff','income',
                   'reputation','income','papers',
                   'impact',
                   'students','staff','co-authorship',
                   'income'],
                    'values':[15,4.5,2.25,6,2.25,18,6,15,30,2.5,2.5,2.5,2.5]})
fig, ax = plt.subplots()
ax = px.sunburst(df,path = ['pillars','subweight'], values = 'values')
fig.show()


#%%
def change_overall(df1, df2):
    merge_df = df1.merge(df2, how='outer' ,left_index = True, right_index = True)
    merge_df.loc['Overall']
    return merge_df
df_EPFL_merge = change_overall(df_EPFL_2017, df_EPFL_2019)   
...
university_2019_overall = concatenate_dfs[(concatenate_dfs['level_0'] == '2019') & (concatenate_dfs['subject'] == 'Overall')].groupby('university').apply(lambda row: 
        row[['university','subject','teaching_score','research_score', 'citation_score','industry_score','international_score']]).set_index('university')

values_NUS = university_2019_overall.loc['National University of Singapore'][1:]
values_NUS = values_NUS.append(pd.Series(values_NUS[0]))
...
plt.xticks(angles, pillars_list, color='grey', size=10)
...
#locs[0].set_horizontalalignment = 'left'
#for loc in loc:
    #loc.
# Draw ylabels
ax.set_rlabel_position(0)
...
# Plot data
ax.plot(angles, values_NUS, linewidth=1, linestyle='solid')
 
# Fill area
ax.fill(angles, values_NUS, 'b', alpha=0.1)
# Show the graph
plt.show()
...
#%%
#rank the university by department
len(concatenate_dfs['subject'].unique())
overall_ranking_df = concatenate_dfs[concatenate_dfs['subject'] == 'Overall'].set_index('university').sort_values(by = 'rank_label')

AH_ranking_2017 = df_2017[df_2017['subject'] == 'Arts and Humanities'].set_index('subject').sort_values(by = 'rank_label')
AH_ranking_2018 = df_2018[df_2018['subject'] == 'Arts and Humanities'].set_index('subject').sort_values(by = 'rank_label')
AH_ranking_2019 = df_2019[df_2019['subject'] == 'Arts and Humanities'].set_index('subject').sort_values(by = 'rank_label')

concatenate_dfs = pd.concat([overall_ranking_2017, overall_ranking_2018, overall_ranking_2019], keys = ['2017','2018','2019']).reset_index(level = 0)

df_2018[df_2018['subject'] == 'Arts and Humanities'].set_index('subject').sort_values(by = 'rank_label').astype(int)
df_2018[df_2018['subject'] == 'Arts and Humanities']['rank_label'].astype(int).sort_values()
...
#%%
df_2017['rank_label'].dtype
stringa = 'missing'
stringa_2 = '123–125'
re.findall("([\d]*)–([\d]*)|([\d]*\.\d)–([\d]*\.\d)", stringa_2)
if re.match("[\d]*–[\d]*", str(stringa)):
    print('yes')
else:
    print('no')
for digit in re.finditer("([\d]*)–([\d]*)|([\d]*\.\d)–([\d]*\.\d)", stringa_2):

    print(digit.groups())
if re.findall("[\d]*–[\d]*|[\d]*\.\d–[\d]*\.\d", stringa):
    print('yes')

df_2017['rank_label'].replace('\D','',regex = True, inplace = True)
df_2017['rank_label'] = df_2017['rank_label'].astype(int)

for col in df_2017.columns[2:]:
    if df_2017[col].dtype == 'object':
        df_2017[col].replace('\w', np.nan, regex = True, inplace = True)

df_2017.groupby('university')['rank_label'].mean()

NUS_ranks = df_2017[df_2017['university'] == 'National University of Singapore']['rank_label']
plt.hist(NUS_ranks, bins = len(NUS_ranks))

overall_ranking_2017 = df_2017.groupby(['university']).apply(lambda row: row[row['subject'] == 'Overall']['rank_label'])

overall_ranking_2017.loc['Carnegie Mellon University']

    
df_2017['overall_score'].dtype
df_2017['overall_score'] = df_2017['overall_score'].apply(replace_range)
df_2018['rank_label'] = df_2018['rank_label'].apply(replace_range)

merging_2017_2018 = overall_ranking_2017.merge(overall_ranking_2018, how='outer',left_index = True, right_index = True, suffixes=('_2017', '_2018'))
merging_all_years = merging_2017_2018.merge(overall_ranking_2019, how='outer',left_index = True, right_index = True, suffixes=('2017', '_2019'))
#overall_ranking_2017.rename(index={'Hong Kong University of Science and Technology':'The Hong Kong University of Science and Technology'},inplace=True)

#overall_ranking_2017 = df_2017[df_2017['subject'] == 'Overall'].set_index('university').sort_values(by = 'rank_label')
#overall_ranking_2018 = df_2018[df_2018['subject'] == 'Overall'].set_index('university').sort_values(by = 'rank_label')
#overall_ranking_2019 = df_2019[df_2019['subject'] == 'Overall'].set_index('university').sort_values(by = 'rank_label')
